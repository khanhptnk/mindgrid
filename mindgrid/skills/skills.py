...
class Primitive(BaseSkill):
    TEMPLATE = {
        "pickup": "pick up the object in the forward cell",
...
class GoTo(BaseSkill):
    ...
    @staticmethod
    def describe():
        return "traverse to a location on the grid. Example: go to column 4 row 3."
...
class RotateTowardsObject(BaseSkill):
    ...
    def verbalize(self, env):
        return self.TEMPLATE.format(
            obj=self.obj.type, col=self.obj.cur_pos[0], row=self.obj.cur_pos[1]
        )
        ...
        for o in first_s.objects:
            if o.type == "door" and not o.is_open:
                for oo in last_s.objects:
                    if oo.type == o.type and oo.init_pos == o.init_pos and oo.is_open:
...
class FixBridge(BaseSkill):
    ...
    def verbalize(self, env):
        return self.TEMPLATE.format(
            col=self.bridge.cur_pos[0], row=self.bridge.cur_pos[1]
        )

    @staticmethod
    def describe():
        return "fix a specific bridge. Example: fix the damaged bridge at column 7."
