        ...
        for placeholder in placeholders:
            if placeholder in ["col", "row"]:
                pattern = pattern.replace(
                    f"{{{placeholder}}}", f"(?P<{placeholder}>\d)"
                )
...
class AddOpeningEdit(DynamicEdit):
    ...
    class TreasureIslandEdit(BaseEdit):
        TEMPLATE = "a new {state} bridge has been constructed at column {col} row {row}"
...
class ToggleOpeningEdit(DynamicEdit):
    class RoomDoorKeyEdit(BaseEdit):
        TEMPLATE = (
            "the door at column {col} row {row} is no longer in the original state"
        )
    ...
    class TreasureIslandEdit(BaseEdit):
        TEMPLATE = (
            "the bridge at column {col} row {row} is no longer in the original state"
        )
